...
|------|------|
| `↑` `↓` `←` `→` | カーソル移動 |
| `Ctrl+H` / `Ctrl+J` / `Ctrl+K` / `Ctrl+L` | 左 / 下 / 上 / 右へ（ホームポジションで移動） |
| `Ctrl+Shift+H/J/K/L` | 同上 + 選択範囲を拡張 |
| `Tab` / `Shift+Tab` | 右 / 左へ移動 |
| `Enter` / `Shift+Enter` | 下 / 上へ移動 |

### macOS で Home/End キーがない場合

Mac の Magic Keyboard など Home/End / PgUp/PgDn がない環境では、
**`Fn+矢印`** を使ってください。OS レベルで標準の Home/End/PgUp/PgDn として
送られるので、追加設定なしでそのまま動きます。
...
A1 / 最終データセルへは `Ctrl+Home` / `Ctrl+End`（Mac では `Fn+Ctrl+←` / `Fn+Ctrl+→`）で移動できます。

### macOS Terminal.app で `Shift+↑/↓` が効かない場合

macOS 標準の Terminal.app はデフォルトプロファイルで `Shift+↑` / `Shift+↓` の
SHIFT 修飾子をプログラムに渡さないため、上下方向の範囲拡張ができません
（左右は問題なし）。次のいずれかで対応してください。

- **代替キー**: `Ctrl+Shift+K`（上に拡張）/ `Ctrl+Shift+J`（下に拡張）
- **Terminal.app 側で修正**: 設定 → プロファイル → 該当プロファイル → キーボード
  タブの `+` で以下を追加
  - キー `↑` / 修飾子 `Shift` / アクション `テキストを送信...` / 値 `\033[1;2A`
  - キー `↓` / 修飾子 `Shift` / アクション `テキストを送信...` / 値 `\033[1;2B`
- **別ターミナルを使う**: iTerm2 / WezTerm / Alacritty / kitty では設定不要で動作
