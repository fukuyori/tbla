...
- **データ**: 並べ替え… / フィルター… / フィルター解除
- **書式**: 列幅を自動調整 / 列幅を広げる/狭める / 列幅を変更… / 太字切替 / 左中央右揃え / 文字色 / 背景色 / 数値書式… / 書式クリア / 条件付き書式…
- **ヘルプ**: キー操作一覧 / バージョン情報
...
| 書式 → 文字色 / 背景色 | RGB 入力ダイアログ（`255,200,200` / `#fee` / `#ffeedd` 形式、空入力でクリア） |
| 書式 → 数値書式… | 種別（general/number/currency/percent/scientific/date/text）と小数桁数を指定 |
...
- **xlsx 読み込み**：背景色・文字色・水平揃え・太字を取り込み（`xl/styles.xml` を直接パース）。**条件付き書式（cellIs 比較、colorScale、dataBar）も読み込み対応** — Excel/LibreOffice で作った書式付き / 条件付き書式付きのファイルがそのまま色付きで開けます。罫線・斜体・下線・テーマ色は現状未対応
